...
class ReassignedDataset(data.Dataset):
    ...
    def __init__(self, image_indexes, pseudolabels, dataset, transform, distance):
...
def cluster_assign(images_lists, dataset, weights):
    ...
    return ReassignedDataset(image_indexes, pseudolabels, dataset, t, uncertainty)
...
def run_kmeans(x, nmb_clusters, verbose=False):
    ...
    return [int(n[0]) for n in I], losses[-1], [float (s[0]) for s in D]
...
class Kmeans(object):
    ...
    def cluster(self, data, verbose=False):
        ...
        # cluster the data
        I, loss, self.D = run_kmeans(xb, self.k, verbose)
        ...
        for i in range(len(self.distance_lists)):
            ...
            if(len(self.distance_lists[i])>1 and self.distance_minmax_lists[i][1] > self.distance_minmax_lists[i][0]):
                for j in range(len(self.distance_lists[i])):
                    # print(self.distance_lists[i][j], self.distance_minmax_lists[i][0], self.distance_minmax_lists[i][1])
                    self.distance_lists[i][j] = (((self.distance_lists[i][j] -  self.distance_minmax_lists[i][0]) /(self.distance_minmax_lists[i][1]- self.distance_minmax_lists[i][0]))/4) + 0.875
        if verbose:
